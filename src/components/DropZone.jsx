...
export default function Dropzone(props) {
  ...
  const generateImage = async (photo) => {
    try {
      ...
      let newPhoto = await getStatus(response.data.id);
  ...
  async function getStatus(id) {
    ...
    if (res.data.status !== "succeeded") {
      ...
      console.log(res.data.output);
      setRestoredImg(res.data.output);
    }
  }
  ...
  return (
    <div
      {...getRootProps()}
      className="w-6/12 mt-1 flex justify-center rounded-md border-2 border-dashed border-gray-300 hover:border-gray-700 px-6 pt-5 pb-6"
    >
